...
export function QuotationDocument({ quotation, project, companySettings, bank }: QuotationDocumentProps) {
  ...
  return (
    <div className="pdf-container">
                  ...
                  <tbody className="divide-y divide-slate-100 text-slate-700">
                     {items.map((item: QuotationItem, i: number) => (
                        <tr key={i} className="align-top">
                           ...
                           <td className="py-4 text-right text-xs font-medium text-slate-800 nums">INR {(item.unit_price ?? 0).toLocaleString('en-IN')} {item.unit ? `/ ${item.unit}` : ''}</td>
      ...
      <div className="pdf-page bg-white text-slate-800 shadow-2xl border border-slate-200/60 rounded-xl overflow-hidden flex flex-col p-10 md:p-12 relative min-h-[900px]">
         ...
         <div className="space-y-8">
            ...
            {bank && (
               <div className="border-t border-slate-100 pt-6 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-[11px] leading-relaxed text-slate-600 bg-slate-50/50 p-4 rounded-xl border border-slate-200/60">
                     <div>
