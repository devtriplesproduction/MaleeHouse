...
import {
  ...
  ArrowUp, ArrowDown, Tag, Lock, Building2, User, Phone, Mail, MapPin,
} from 'lucide-react';
...
export function QuotationBuilderEngine({
  ...
  const handleTemplateChange = (templateId: string) => {
    ...
    if (!templateId) { setActiveClauses([]); return; }
    const tpl = templates.find((t: any) => t.id === templateId);
    if (tpl) setActiveClauses(JSON.parse(JSON.stringify(tpl.clauses)));
  };
            ...
            <div className="px-5 py-3 border-b border-slate-200 dark:border-white/10 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-slate-50 dark:bg-white/[0.02]">
              ...
              <div className="flex items-center gap-2">
                <select value={selectedTemplateId} onChange={e => handleTemplateChange(e.target.value)}
                  className="h-8 px-3 rounded-lg bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 text-xs text-slate-600 dark:text-slate-400 outline-none">
