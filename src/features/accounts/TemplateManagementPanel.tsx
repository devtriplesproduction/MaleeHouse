    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold text-slate-900 dark:text-white">Viewing Template: {viewingTemplate.name}</h2>
            <p className="text-xs text-slate-500 mt-1">Category: {viewingTemplate.category}</p>
          </div>
          <button 
            onClick={() => setViewingTemplate(null)}
            className="flex items-center gap-2 px-4 py-2 bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 text-slate-600 dark:text-slate-300 text-sm font-semibold rounded-xl transition-all"
          >
        ...
        <div className="glass-card p-6 space-y-6">
          <div className="space-y-4">
            {(viewingTemplate.clauses || []).map((c: any, index: number) => (
              <div key={c.id || index} className="p-4 rounded-xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/[0.02]">
                <h4 className="font-semibold text-slate-800 dark:text-slate-200 text-sm mb-2">{c.display_order}. {c.title}</h4>
                <p className="text-sm text-slate-600 dark:text-slate-400 whitespace-pre-wrap leading-relaxed">{c.content}</p>
              </div>
            ))}
            {(!viewingTemplate.clauses || viewingTemplate.clauses.length === 0) && (
              <p className="text-sm text-slate-500 text-center py-8">No clauses defined.</p>
            )}
          </div>
        </div>
      </div>
