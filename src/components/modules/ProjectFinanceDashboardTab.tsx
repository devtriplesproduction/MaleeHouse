...
export function ProjectFinanceDashboardTab({ 
  ...
  const finances = projectFinances || null;
  
  // Transform income
  let incomeItems: LedgerItem[] = [];
  if (Array.isArray(projectInvoices)) {
    incomeItems = incomeItems.concat(projectInvoices.map((inv: any) => ({
      id: inv.id,
      date: inv.created_at,
  ...
  if (Array.isArray(projectPayments)) {
    incomeItems = incomeItems.concat(projectPayments.map((pay: any) => ({
      id: pay.id,
      date: pay.created_at,
  ...
  // Transform expenses
  let expenseItems: LedgerItem[] = [];
  if (Array.isArray(projectExpenses)) {
    expenseItems = projectExpenses.map((exp: any) => ({
      id: exp.id,
      date: exp.date,
