...
export function LeaveForm({ initialHolidays = [], initialBalance = null, initialMyLeaves = [] }: LeaveFormProps = {}) {
  ...
  const [leaveBalance, setLeaveBalance] = useState<number | null>(initialBalance);
  const [baseBalance, setBaseBalance] = useState<number | null>(initialBalance);
  const [myLeaves, setMyLeaves] = useState<any[]>(initialMyLeaves);

  // Sync state if props change from router.refresh()
  useEffect(() => {
    setHolidays(initialHolidays);
    setBaseBalance(initialBalance);
    setMyLeaves(initialMyLeaves);
  }, [initialHolidays, initialBalance, initialMyLeaves]);
