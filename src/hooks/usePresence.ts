...
export function usePresence() {
  ...
  // below, and the effect itself calls setOnlineUsers — so every state
  // update caused a re-render, which created a new supabase reference,
  // which re-ran the effect, tearing down and re-subscribing the presence
  // channel on every render. useState(() => ...) creates the client
  // exactly once per hook instance (same fix already applied in
  // UserProvider/RealtimeProvider).
